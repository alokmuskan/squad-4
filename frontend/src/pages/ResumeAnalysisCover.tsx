...
interface ResumeAnalysisCoverProps {
  onUploadResume: () => void;
  onBack: () => void;
}
    ...
    Back
      ...
      <section className="relative z-10 mx-auto grid min-h-[calc(100vh-7rem)] w-full max-w-7xl items-center gap-11 lg:grid-cols-[0.92fr_1.08fr] lg:gap-16 xl:gap-20">
        ...
        >
          ...
          <motion.h1
            variants={fadeUp}
            className="max-w-xl text-[clamp(3rem,8vw,5.4rem)] font-black leading-[0.92] tracking-normal text-[#07144a]"
          >
            Resume
            <br />
            Analysis
          ...
          <motion.p
            variants={fadeUp}
            className="mt-7 max-w-lg text-lg font-medium leading-8 text-[#53627e] sm:text-xl"
          >
          ...
          <motion.ul
            ...
            aria-label="Resume analysis features"
          >
            {features.map((feature) => {
              ...
              return (
                <li
                  className="flex items-center gap-4 text-base font-bold text-[#263454] sm:text-lg"
                  key={feature.label}
                >
                  <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-[#e9ebff] text-[#5a5cff]">
          ...
          <motion.div
            variants={fadeUp}
            className="mt-9 flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-6"
          >
            <Button
              type="button"
              size="lg"
              className="h-14 rounded-xl bg-[#585cff] px-8 text-base font-extrabold shadow-[0_18px_28px_rgba(85,91,255,0.28)] transition hover:-translate-y-0.5 hover:bg-[#4868ff] sm:h-16 sm:text-lg"
            ...
            <Button
              ...
              size="lg"
              variant="outline"
              className="h-14 rounded-xl border-2 border-[#5a5cff] bg-white/60 px-8 text-base font-extrabold text-[#5a5cff] transition hover:-translate-y-0.5 hover:bg-white sm:h-16 sm:text-lg"
            >
              View Sample Report
            </Button>
          </motion.div>
      ...
      <motion.div
        className="relative z-10 mx-auto mt-6 flex w-max max-w-full items-center gap-3 text-sm font-bold text-[#5d6b88] sm:text-base lg:-mt-4"
        initial={{ opacity: 0, y: 12 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.75, duration: 0.45 }}
      >
        <ShieldCheck className="h-5 w-5 shrink-0" />
        <span className="truncate">Trusted by 10,000+ professionals</span>
      </motion.div>
    </main>
  );
}
...
function Leaf({ className }: { className: string }) {
  return (
    <span
      className={`absolute h-[8%] w-[16%] rounded-[100%_0_100%_0] ${className}`}
    />
  );
}
