...
export const Route = createFileRoute("/_authenticated/RoadmapVisualization")({
...
function RoadmapVisualization() { 
  ...
  const phases = [
    { name: "Foundations", skills: ["HTML", "CSS", "JavaScript"], status: "completed" },
    { name: "Frontend Development", skills: ["React", "State Management"], status: "completed" },
    { name: "Backend Development", skills: ["Node.js", "Express", "MongoDB"], status: "inProgress" },
    { name: "Cloud & Deployment", skills: ["Docker", "AWS", "CI/CD"], status: "locked" },
  ];
  ...
  const getStatusColor = (status: string): string => {
    switch (status) {
      ...
      case "inProgress": return "bg-yellow-100 text-yellow-700 border-yellow-300";
      case "upcoming": return "bg-blue-100 text-blue-700 border-blue-300";
      case "locked": return "bg-gray-100 text-gray-600 border-gray-300";
    ...
    <div className="min-h-screen bg-[#F8F6E8] pb-20">
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-lg border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <button onClick={() => navigate({ to: "/" })}  className="px-5 py-3 bg-white shadow-lg rounded-2xl hover:scale-105 transition">
            ← Back
          </button>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="mb-12">
          <h1 className="text-5xl font-black text-[#171C4A] mb-3">Roadmap Visualization</h1>
          <p className="text-lg text-gray-600">
            Your personalized AI-generated learning journey from your current skills to your dream career.
          </p>
        </div>
          ...
          <div className="bg-gradient-to-br from-[#171C4A] to-blue-900 rounded-[40px] p-10 text-white shadow-2xl overflow-hidden relative">
            ...
            <div className="relative z-10">
              <div className="grid md:grid-cols-5 gap-4 items-center">
                ...
                <div className="flex justify-center"><div className="text-3xl animate-bounce">→</div></div>
              ...
              <div className="mt-8 bg-white/10 rounded-2xl p-6 border border-white/20">
                <div className="grid md:grid-cols-2 gap-6">
                  <div><div className="text-sm opacity-80">Estimated Completion</div><div className="text-3xl font-black mt-1">10 Weeks</div></div>
                  <div><div className="text-sm opacity-80">Remaining Time</div><div className="text-3xl font-black mt-1">5 Weeks</div></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="mb-12">
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {[
              { icon: "✔", count: "4", label: "Completed Skills", color: "text-green-600" },
              { icon: "🔄", count: "1", label: "In Progress", color: "text-yellow-600" },
              { icon: "⭐", count: "2", label: "Upcoming Skills", color: "text-blue-600" },
              { icon: "🔒", count: "2", label: "Locked Skills", color: "text-gray-600" },
            ].map((item) => (
              <div key={item.label} className="bg-white rounded-[32px] p-6 shadow-xl text-center">
          ...
          <div className="bg-white rounded-[32px] p-8 shadow-xl">
            <div className="space-y-4">
              {roadmapSkills.map((skill, index) => (
                <div key={skill.name}>
                  <div className={`rounded-2xl p-4 border-2 cursor-pointer hover:shadow-lg transition-all ${getStatusColor(skill.status)}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className="text-2xl">{getStatusIcon(skill.status)}</span>
                        <div>
                          <div className="font-bold">{skill.name}</div>
                          <div className="text-xs opacity-70">{skill.duration} • {skill.difficulty}</div>
                        </div>
                      </div>
                      <span className="text-sm font-semibold capitalize px-3 py-1 bg-white/50 rounded-full">
                        {skill.status === "inProgress" ? "In Progress" : skill.status === "completed" ? "Done" : skill.status === "upcoming" ? "Coming" : "Locked"}
                      </span>
                    </div>
                  </div>
                  {index < roadmapSkills.length - 1 && <div className="flex justify-center py-2"><div className="text-2xl text-gray-400">↓</div></div>}
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="mb-12">
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {phases.map((phase) => (
              <div key={phase.name} className={`rounded-[32px] p-8 shadow-xl transition-all ${
                phase.status === "completed" ? "bg-green-50 border-2 border-green-300" :
                phase.status === "inProgress" ? "bg-yellow-50 border-2 border-yellow-300" :
                "bg-gray-50 border-2 border-gray-300"
              }`}>
                <div className="flex items-center gap-2 mb-4">
                  <span className="text-3xl">{phase.status === "completed" ? "✔" : phase.status === "inProgress" ? "🔄" : "🔒"}</span>
                  <h3 className="text-xl font-bold text-[#171C4A]">{phase.name}</h3>
                </div>
                <div className="space-y-2">
                  {phase.skills.map((skill) => (
                    <div key={skill} className={`text-sm font-semibold px-3 py-1 rounded-full ${
                      phase.status === "completed" ? "bg-green-200 text-green-700" :
                      phase.status === "inProgress" ? "bg-yellow-200 text-yellow-700" :
        ...
        <section className="mb-12">
          ...
          <div className="bg-white rounded-[32px] p-8 shadow-xl">
            <div className="space-y-4">
              {[
                { skill: "Docker", prereqs: ["Node.js", "Git"], duration: "7 Days", difficulty: "Medium" },
                { skill: "AWS", prereqs: ["Docker", "Linux"], duration: "10 Days", difficulty: "Hard" },
                { skill: "MongoDB", prereqs: ["JavaScript", "Node.js"], duration: "5 Days", difficulty: "Medium" },
              ].map((item) => (
                <div key={item.skill} className="bg-[#F8F6E8] rounded-2xl p-6 hover:shadow-lg transition-shadow">
                  ...
                  <div className="grid md:grid-cols-4 gap-4">
                    <div>
                      ...
                      <div className="mt-2 space-y-1">{item.prereqs.map((p) => <div key={p} className="text-sm font-semibold">✔ {p}</div>)}</div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div className={`text-lg font-bold mt-2 ${item.difficulty === "Hard" ? "text-red-600" : item.difficulty === "Medium" ? "text-yellow-600" : "text-green-600"}`}>{item.difficulty}</div>
                    </div>
                    <button className="bg-[#F2DD85] text-[#171C4A] font-bold rounded-xl px-4 py-2 hover:opacity-90 transition">Learn More</button>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className="space-y-8">
                {weeklyTimeline.map((item) => (
                  <div key={item.week} className="flex gap-6 items-start">
                    <div className="relative z-10 bg-[#F2DD85] rounded-full w-24 h-24 flex items-center justify-center font-bold text-[#171C4A] flex-shrink-0 shadow-lg">Week {item.week}</div>
                    <div className="bg-[#F8F6E8] rounded-2xl p-6 flex-1">
                      ...
                      <button className="mt-4 bg-[#F2DD85] text-[#171C4A] px-4 py-2 rounded-full font-semibold hover:opacity-90 transition">View Details</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="mb-4">
                ...
                <div className="space-y-1">
                  {["✔ HTML", "✔ CSS", "✔ JavaScript", "✔ React"].map((s) => (
                    <span key={s} className="inline-block bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm font-semibold mr-2 mb-2">{s}</span>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-white rounded-[32px] p-8 shadow-xl border-2 border-yellow-300">
              ...
              <div className="mb-4">
                ...
                <div className="space-y-1">
                  <span className="inline-block bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full text-sm font-semibold mr-2 mb-2">🔄 Node.js</span>
                  <span className="inline-block bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm font-semibold mr-2 mb-2">🔒 MongoDB</span>
                  <span className="inline-block bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm font-semibold mr-2 mb-2">🔒 Docker</span>
                </div>
              </div>
              <div className="text-center py-4 bg-yellow-50 rounded-2xl">
                <div className="text-sm font-bold text-yellow-700">42% COMPLETE</div>
                <div className="mt-2 h-2 bg-gray-200 rounded-full"><div className="h-2 bg-yellow-400 rounded-full w-[42%]"></div></div>
              </div>
            </div>
          </div>
        </section>
            ...
            <div className="space-y-4">
              {projects.map((project) => (
                <div key={project.name}>
                  <div className={`rounded-2xl p-4 border-2 ${getStatusColor(project.status)}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                  ...
                  {projects.indexOf(project) < projects.length - 1 && <div className="flex justify-center py-2"><div className="text-2xl text-gray-400">↓</div></div>}
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="mb-12">
          ...
          <div className="bg-gradient-to-br from-blue-600 to-purple-700 rounded-[40px] p-10 text-white shadow-2xl">
            <div className="grid md:grid-cols-3 gap-8">
              <div className="text-center">
                ...
                <div className="relative w-40 h-40 mx-auto mb-4">
                  <svg className="w-full h-full transform -rotate-90" viewBox="0 0 120 120">
                    ...
                    <circle cx="60" cy="60" r="50" fill="none" stroke="white" strokeWidth="6"
                      strokeDasharray={`${(72 / 100) * 314} 314`} strokeLinecap="round" />
                  </svg>
              ...
              <div className="flex items-center justify-center"><div className="text-4xl">→</div></div>
              <div className="text-center">
                <div className="text-sm opacity-80 mb-2">Predicted Readiness (After Skills)</div>
                <div className="relative w-40 h-40 mx-auto mb-4">
                  <svg className="w-full h-full transform -rotate-90" viewBox="0 0 120 120">
                    <circle cx="60" cy="60" r="50" fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="6" />
                    <circle cx="60" cy="60" r="50" fill="none" stroke="#F2DD85" strokeWidth="6"
                      strokeDasharray={`${(88 / 100) * 314} 314`} strokeLinecap="round" />
                  </svg>
                  <div className="absolute inset-0 flex items-center justify-center">
            ...
            <div className="mt-8 bg-white/10 rounded-2xl p-6 border border-white/20 text-center">
              ...
              <div className="text-white/80 mt-2">Master Docker, AWS, and CI/CD to boost your Full Stack Developer readiness</div>
            </div>
          </div>
        </section>
              ...
              <div key={item.skill} className="bg-white rounded-[32px] p-6 shadow-xl hover:shadow-2xl transition-shadow">
                ...
                <div className="space-y-2">
                  {item.resources.map((resource) => (
                    <button key={resource} className="w-full text-left px-4 py-2 bg-[#F8F6E8] hover:bg-[#F2DD85] rounded-lg font-semibold text-[#171C4A] transition">{resource}</button>
                  ))}
                </div>
              </div>
            ))}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={item.category} className="bg-white rounded-[32px] p-6 shadow-xl">
                ...
                <div className="mb-4">
                  <div className="flex justify-between mb-2"><span className="text-sm font-semibold">{item.progress}%</span></div>
                  <div className="w-full bg-gray-200 rounded-full h-4">
                    <div className={`h-4 rounded-full transition-all ${
                      item.progress >= 80 ? "bg-green-500" : item.progress >= 60 ? "bg-yellow-500"
        ...
        <section className="mb-12">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={action.title} className="bg-white rounded-[32px] p-6 shadow-xl hover:shadow-2xl transition-shadow">
                <div className="flex items-start justify-between gap-6">
                  <div className="flex-1">
                    ...
                    <div className="flex gap-3 items-center">
                      <button className="bg-[#F2DD85] text-[#171C4A] px-6 py-2 rounded-full font-bold hover:opacity-90 transition">Start</button>
