...
// FIXED: Path alias modified to relative imports
...
async function buildRichSystemPrompt(userId: string): Promise<string> {
    ...
    if (userRecord) {
        ...
        // FIXED: Added type definitions for reduction context matching
        const totalStars = repositories.reduce((sum: number, r: { starsCount: number }) => sum + r.starsCount, 0);
        ...
        const topLangs = [...langCounts.entries()]
            .sort((a, b) => b[1] - a[1])
            .slice(0, 5)
            .map(([lang]: [string, number]) => lang); // FIXED: Added explicit mapping loop typing
...
export async function getUserSessions(userId: string) {
    ...
    // FIXED: Added types to map step iteration parameter context
    return sessions.map((s: { id: string; title: string; _count: { messages: number }; messages: { createdAt: Date }[]; createdAt: Date }) => ({
        id: s.id,
        title: s.title,
...
export async function getSessionMessages(userId: string, sessionId: string) {
    ...
    return messages.map((m: { id: string; sessionId: string; role: string; content: string; createdAt: Date }) => ({
        id: m.id,
